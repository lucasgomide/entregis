RSpec.describe ShippingCarriers::UpdateOperation, type: :operation do
  subject(:operation) { described_class.new }
  let(:updating_contract) { instance_spy(ShippingCarriers::UpdatingContract) }

  let(:app_container_stubs) do
    Entregis::Container.stub('shipping_carriers.updating_contract', updating_contract)
  end
  ...
  describe '.call' do
    ...
    context 'when resource updating has failed' do
      let(:input) { { id: resource.id, name: nil } }
    ...
    context 'when resource was successful updated' do
      ...
      it do
        expect { call }.to change { resource.reload.name }.to(input[:name])
      end

      it do
        expect { call }.to change { resource.reload.document }.to(input[:document])
      end
    end
  end
end
