RSpec.describe Carriers::CreationBuilder, type: :builder do
  ...
  describe '.build_attributes' do
    ...
    context 'when the process to convert coordinates to RGeo Point has successful' do
      ...
      its([:current_location]) { is_expected.to be_an_instance_of(RGeo::Cartesian::PointImpl) }
      its([:coverage_area]) { is_expected.to be_an_instance_of(RGeo::Cartesian::MultiPolygonImpl) }
    end
  end
end
